...
__global__ __launch_bounds__(Q4RDNA_THREADS) void q4rdna_gemv_old_mapping(
    ...
    __shared__ float activation_tile[Q4RDNA_WARPS][Q4RDNA_GROUP];
    ...
    const int row = row_tile * Q4RDNA_TILE_ROWS + lane;
    const int groups = columns / Q4RDNA_GROUP;
    float sum = 0.0f;
    float gate_sum = 0.0f;

    for (int group = 0; group < groups; ++group) {
        activation_tile[warp][lane] = activation[group * Q4RDNA_GROUP + lane];
        activation_tile[warp][lane + Q4RDNA_TILE_ROWS] =
            activation[group * Q4RDNA_GROUP + lane + Q4RDNA_TILE_ROWS];
        __syncwarp();

        const uint8_t * tile = weights + uint64_t(row_tile * groups + group) * Q4RDNA_TILE_BYTES;
        const float scale = __half2float(reinterpret_cast<const half *>(tile)[lane]);
        ...
        if constexpr (use_gate) {
            ...
            gate_scale = __half2float(reinterpret_cast<const half *>(gate_tile)[lane]);
        }
#pragma unroll
        for (int i = 0; i < Q4RDNA_GROUP / 2; ++i) {
            const uint8_t packed = tile[64 + i * Q4RDNA_TILE_ROWS + lane];
            sum = fmaf(scale * float(q4rdna_low(packed)), activation_tile[warp][2 * i], sum);
            sum = fmaf(scale * float(q4rdna_high(packed)), activation_tile[warp][2 * i + 1], sum);
            if constexpr (use_gate) {
                const uint8_t gate_packed = gate_tile[64 + i * Q4RDNA_TILE_ROWS + lane];
                gate_sum = fmaf(
                    gate_scale * float(q4rdna_low(gate_packed)), activation_tile[warp][2 * i], gate_sum);
                gate_sum = fmaf(
                    gate_scale * float(q4rdna_high(gate_packed)), activation_tile[warp][2 * i + 1], gate_sum);
            }
        }
        __syncwarp();
    }
    if constexpr (use_add) sum += add[row];
    if constexpr (use_gate) sum *= ggml_cuda_op_silu_single(gate_sum);
    output[row] = sum;
}
...
__global__ __launch_bounds__(Q4RDNA_THREADS) void q4rdna_gemv_split_k(
        const uint8_t * weights, const uint8_t * gate_weights, const float * activation,
        const float * add, float * output, int columns) {
    ...
    __shared__ float partial_shared[Q4RDNA_WARPS][Q4RDNA_TILE_ROWS];
    __shared__ float gate_partial_shared[use_gate ? Q4RDNA_WARPS : 1][Q4RDNA_TILE_ROWS];
    const int lane = threadIdx.x & (Q4RDNA_TILE_ROWS - 1);
    const int warp = threadIdx.x / Q4RDNA_TILE_ROWS;
...
#pragma unroll 4
        for (int i = 0; i < Q4RDNA_GROUP / 2; ++i) {
            ...
            const float activation_high = activation[group * Q4RDNA_GROUP + 2 * i + 1];
            const uint8_t packed = tile[64 + i * Q4RDNA_TILE_ROWS + lane];
            partial_low = fmaf(float(q4rdna_low(packed)), activation_low, partial_low);
            partial_high = fmaf(float(q4rdna_high(packed)), activation_high, partial_high);
            if constexpr (use_gate) {
                const uint8_t gate_packed = gate_tile[64 + i * Q4RDNA_TILE_ROWS + lane];
                gate_partial_low =
                    fmaf(float(q4rdna_low(gate_packed)), activation_low, gate_partial_low);
                gate_partial_high =
                    fmaf(float(q4rdna_high(gate_packed)), activation_high, gate_partial_high);
            }
        }
        const float scale = __half2float(reinterpret_cast<const half *>(tile)[lane]);
        sum = fmaf(scale, partial_low + partial_high, sum);
        if constexpr (use_gate) {
            const float gate_scale = __half2float(reinterpret_cast<const half *>(gate_tile)[lane]);
            gate_sum = fmaf(gate_scale, gate_partial_low + gate_partial_high, gate_sum);
        }
    }

    partial_shared[warp][lane] = sum;
    if constexpr (use_gate) {
        gate_partial_shared[warp][lane] = gate_sum;
    }
    __syncthreads();
...
#pragma unroll
        for (int other = 1; other < split_waves; ++other) {
            sum += partial_shared[warp + other][lane];
            if constexpr (use_gate) {
                gate_sum += gate_partial_shared[warp + other][lane];
            }
        }
        const int row = row_tile * Q4RDNA_TILE_ROWS + lane;
        if constexpr (use_add) sum += add[row];
...
__global__ __launch_bounds__(split_waves * Q4RDNA_TILE_ROWS) void q4rdna_gemv_small_split_k(
    ...
    constexpr int slices_per_tile = Q4RDNA_TILE_ROWS / slice_rows;
    __shared__ float partial_shared[split_waves][Q4RDNA_TILE_ROWS];
    __shared__ float gate_partial_shared[use_gate ? split_waves : 1][Q4RDNA_TILE_ROWS];
    const int lane = threadIdx.x & (Q4RDNA_TILE_ROWS - 1);
    const int warp = threadIdx.x / Q4RDNA_TILE_ROWS;
    ...
    if (lane < slice_rows) {
        for (int group = warp; group < groups; group += split_waves) {
            const uint8_t * tile = weights + uint64_t(row_tile * groups + group) * Q4RDNA_TILE_BYTES;
            const uint8_t * gate_tile = nullptr;
            if constexpr (use_gate) {
                gate_tile = gate_weights + uint64_t(row_tile * groups + group) * Q4RDNA_TILE_BYTES;
...
#pragma unroll 4
            for (int i = 0; i < Q4RDNA_GROUP / 2; ++i) {
                const float activation_low = activation[group * Q4RDNA_GROUP + 2 * i];
                const float activation_high = activation[group * Q4RDNA_GROUP + 2 * i + 1];
                const uint8_t packed = tile[64 + i * Q4RDNA_TILE_ROWS + row_lane];
                partial_low = fmaf(float(q4rdna_low(packed)), activation_low, partial_low);
                partial_high = fmaf(float(q4rdna_high(packed)), activation_high, partial_high);
                if constexpr (use_gate) {
                    const uint8_t gate_packed = gate_tile[64 + i * Q4RDNA_TILE_ROWS + row_lane];
                    gate_partial_low =
                        fmaf(float(q4rdna_low(gate_packed)), activation_low, gate_partial_low);
                    gate_partial_high =
                        fmaf(float(q4rdna_high(gate_packed)), activation_high, gate_partial_high);
            ...
            sum = fmaf(scale, partial_low + partial_high, sum);
            if constexpr (use_gate) {
                const float gate_scale = __half2float(reinterpret_cast<const half *>(gate_tile)[row_lane]);
                gate_sum = fmaf(gate_scale, gate_partial_low + gate_partial_high, gate_sum);
            }
        }
    }
...
#pragma unroll
        ...
        const int row = row_tile * Q4RDNA_TILE_ROWS + row_lane;
        if constexpr (use_add) sum += add[row];
        if constexpr (use_gate) sum *= ggml_cuda_op_silu_single(gate_sum);
        output[row] = sum;
    }
...
void q4rdna_launch_small(
    ...
    q4rdna_gemv_small_split_k<use_gate, use_add, split_waves, slice_rows>
...
void q4rdna_launch_width(
    ...
    q4rdna_gemv_split_k<use_gate, use_add, lanes_per_row>
...
void q4rdna_launch(
    ...
    if (mapping != nullptr && std::strcmp(mapping, "old") == 0) {
        ...
        GGML_ASSERT(rows % rows_per_block == 0);
        q4rdna_gemv_old_mapping<use_gate, use_add>
            <<<rows / rows_per_block, Q4RDNA_THREADS, 0, stream>>>(
                weights, gate_weights, activation, add, output, columns);
        return;
    }
